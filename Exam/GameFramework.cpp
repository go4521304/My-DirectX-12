...
CGameFramework::CGameFramework()
{
	...
	_tcscpy_s(m_pszFrameRate, _T("EXAM ("));
}
...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_LBUTTONDOWN:
		m_pScene->PickObjectPointedByCursor(LOWORD(lParam), HIWORD(lParam), m_pCamera);
		break;
	case WM_RBUTTONDOWN:
		if (m_pPlayer) m_pCamera = m_pPlayer->ChangeCamera((m_pCamera->GetMode() % 2) + 1, m_GameTimer.GetTimeElapsed());
		break;
	...
	case WM_KEYUP:
		...
		case VK_RETURN:
			...
			else
			{
				...
				m_ptOldCursorPos.x = rc.left + (m_nWndClientWidth / 2);
				m_ptOldCursorPos.y = rc.top + (m_nWndClientHeight / 2);
...
void CGameFramework::AnimateObjects()
{
	if (m_pScene)
	{
		m_pScene->AnimateObjects(m_GameTimer.GetTimeElapsed(), m_pPlayer);
	}
}
...
void CGameFramework::FrameAdvance()
{
	...
	m_GameTimer.GetFrameRate(m_pszFrameRate + 6, 37);
