...
D3D12_SHADER_BYTECODE CShader::CompileShaderFromFile(WCHAR* pszFileName, LPCSTR pszShaderName, LPCSTR pszShaderProfile, ID3DBlob** ppd3dShaderBlob)
{
	...
	HRESULT hr = ::D3DCompileFromFile(pszFileName, NULL, NULL, pszShaderName, pszShaderProfile, nCompileFlags, 0, ppd3dShaderBlob, &errorMessages);
...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	// 원래 *3.5f
	float fxPitch = 12.0f * 12.0f;
	float fyPitch = 12.0f * 12.0f;
	float fzPitch = 12.0f * 12.0f;
...
D3D12_SHADER_BYTECODE CBillboardShader::CreateGeometryShader(ID3DBlob** ppd3dShaderBlob, int nPipelineState)
{
...
D3D12_INPUT_LAYOUT_DESC CBillboardShader::CreateInputLayout(int nPipelineState)
{
	UINT nInputElementDescs = 2;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
void CBulletShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	/*파티클 오브젝트 배열 생성위치*/
	m_ppParticleObj = new CParticleObject * [m_nObjects];

	/* CBulletObject 생성하는 부분 */
	/* CPaticleObject도 같이 세팅 해줌 CBulletObject와는 별개의 개체로 CBulletShader에 있음 */
	for (int i = 0; i< m_nObjects; i++)
	{
		...
		/* 파티클 오브젝트 생성 */
		m_ppParticleObj[i] = new CParticleObject(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT2(8.0f, 8.0f), 5.0f, MAX_PARTICLES);
	}
}
...
D3D12_SHADER_BYTECODE CBulletShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob, int nPipelineState)
{
...
void CBulletShader::ReleaseUploadBuffers()
{
	if (m_ppObjects)
	{
		for (int j = 0; j < m_nObjects; j++)
		{
			...
			if (m_ppParticleObj[j]) delete m_ppParticleObj[j];
		}
...
D3D12_INPUT_LAYOUT_DESC CParticleShader::CreateInputLayout(int nPipelineState)
{
	...
	pd3dInputElementDescs[3] = { "LIFETIME", 0, DXGI_FORMAT_R32_FLOAT, 0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[4] = { "PARTICLETYPE", 0, DXGI_FORMAT_R32_UINT, 0, 28, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
D3D12_STREAM_OUTPUT_DESC CParticleShader::CreateStreamOuputState(int nPipelineState)
{
	...
	if (nPipelineState == 0)
	{
		...
		pd3dStreamOutputDecls[1] = { 0, "POSITION", 0, 0, 3, 0 };
		pd3dStreamOutputDecls[2] = { 0, "VELOCITY", 0, 0, 3, 0 };
		pd3dStreamOutputDecls[3] = { 0, "SIZE", 0, 0, 2, 0 };
		pd3dStreamOutputDecls[4] = { 0, "LIFETIME", 0, 0, 1, 0 };
		pd3dStreamOutputDecls[5] = { 0, "PARTICLETYPE", 0, 0, 1, 0 };
