...
CGameFramework::CGameFramework()
{
	...
	_tcscpy_s(m_pszFrameRate, _T("HW2 ("));
}
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	for (int i = 0; i < m_nSwapChainBuffers; i++) m_nFenceValues[i] = 0;	// 내가 추가한거
...
void CGameFramework::BuildObjects()
{
	...
	// 벽 생성
	CWallShader* pWall = new CWallShader(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature());
	m_pWall = pWall;
	...
	m_pCollision = new CCollision(m_pScene, m_pWall, m_pPlayer);
...
void CGameFramework::ProcessInput()
{
	...
	m_pCollision->check_PlayerByWall();
}

void CGameFramework::AnimateObjects()
{
	if (m_pScene) m_pScene->AnimateObjects(m_GameTimer.GetTimeElapsed());
	m_pCollision->check_ObjOverPlayer();
	if (m_pCollision->check_PlayerbyObj() && count == -1)
	{
		count = 5;
		LR = 1;
		angle = 0.0f;
	...
	if (count >= 0)
	{
		float tmp_angle = 80.0f * m_GameTimer.GetTimeElapsed();

		if (LR == 1)
		{
			m_pPlayer->Rotate(0.0f, 0.0f, tmp_angle);
			angle += tmp_angle;
		...
		else
		{
			...
			if (angle < 0.0f)
			{
				m_pPlayer->Rotate(0.0f, 0.0f, -angle);
				count--;
			}
		}
...
void CGameFramework::FrameAdvance()
{
	...
	// 벽개체
	m_pWall->Render(m_pd3dCommandList, m_pCamera);
	...
	m_GameTimer.GetFrameRate(m_pszFrameRate + 5, 37);
