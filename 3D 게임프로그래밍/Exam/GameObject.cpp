...
void CGameObject::Update_Bounding()
{
	if (m_ppMeshes[0])
	{
...
void CRotatingObject::Update_State(CPlayer* player, CHeightMapTerrain* m_pTerrain)
{
	if (HP < 0)
		state = DESTROY;
	...
	if (state != DESTROY)
	{
		if (Vector3::Length(Vector3::Subtract(GetPosition(), player->GetPosition())) < 300.0f)
		{
			state = READY;
		}
		else
	...
	xmf4Color) : CGameObject(0)
{
	...
	m_nWidth = nWidth;
	m_nLength = nLength;
	/*지형 객체는 격자 메쉬들의 배열로 만들 것이다. nBlockWidth, nBlockLength는 격자 메쉬 하나의 가로, 
	세로 크기이다. cxQuadsPerBlock, czQuadsPerBlock은 격자 메쉬의 가로 방향과 세로 방향 사각형의 개수이다.*/
...
{
	DurationTime -= fTimeElapsed;
}

void CBulletObject::SetDuration(float time)
{
	DurationTime = time;
}